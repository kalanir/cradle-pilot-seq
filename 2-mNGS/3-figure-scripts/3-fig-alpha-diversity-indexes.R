...
taxon_full = readRDS(paste0(data_dir, "/Bracken_processed_data/filtered_pathogens/pathogen_species_counts.RDS"))
...
##### GENUS #######
taxon_full = readRDS(paste0(data_dir, "/Bracken_processed_data/filtered_pathogens/pathogen_genus_counts.RDS"))
taxon_percent = readRDS(paste0(data_dir, "/Bracken_processed_data/filtered_pathogens/pathogen_genus_percentages.RDS"))
taxon_percent_dt = as.data.table(taxon_percent)
taxon_percent_dt = as.data.table(taxon_percent)
...
P2 <- ggplot(data_alphadiv_tax, aes(x=sample_type, y=S.chao1)) +
  geom_boxplot(aes(fill = sample_type)) + scale_fill_manual(values = sample_type_color) + geom_line(aes(group = hhid), color = "#707B7C") +
...
combined_alpha_fig
ggsave(combined_alpha_fig, filename = paste0(figure_path, "fig-pathogen-alpha-species-bracken",".png"), height = 6, width = 8.5)
...
P5 <- ggplot(data_alphadiv_genus, aes(x=sample_type, y=data_simpson)) +
  geom_boxplot(aes(fill = sample_type)) + scale_fill_manual(values = sample_type_color) + geom_line(aes(group = hhid), color = "#707B7C") +
  labs(title= 'Simpson', x= ' ', y= '', tag = "E") +
  geom_point() + labs(fill = "Sample Type") + annotate("text", x = -Inf, y = -Inf, hjust = -0.1, vjust = -1.0, label = paste0("p = ",round(p5_stat$p.value, 3)), size = 3)
...
combined_alpha_fig = combined + plot_layout(guides = "collect")
combined_alpha_fig
ggsave(combined_alpha_fig, filename = paste0(figure_path, "fig-pathogen-alpha-genus-bracken",".png"), height = 6, width = 8.5)
